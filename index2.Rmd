...
plot1 <- nyts_data %>%
    dplyr::group_by(year) %>%
    dplyr::summarize("Ever_Any Tobacco Product"=(mean(tobacco_ever, na.rm = TRUE)*100),
                     "Current_Any Tobacco Product"=(mean(tobacco_current, na.rm = TRUE)*100),
...
nyts_data %>%
    dplyr::group_by(year) %>%
    dplyr::summarize("Ever e-cig"=(mean(ecig_only_ever, na.rm = TRUE)*100),
                     "Current e-cig"=(mean(ecig_only_current, na.rm = TRUE)*100),
                     "Ever non-e-cig" =(mean(non_ecig_only_ever, na.rm = TRUE)*100),
                     "Current non-e-cig"=(mean(non_ecig_only_current, na.rm = TRUE)*100)) %>%
  pivot_longer(cols = -year, names_to = "User", values_to = "Percentage of students") %>%
...
plot1c <- nyts_data %>%
  ...
  # this moves the legend to the bottom of the plot and removes the x axis title
    theme(legend.position = "bottom",
          axis.title.x = element_blank()) +
    labs(title = "How has e-cigarette use and tobacco use (besides e-cigarettes) varied over time?",
         y = "% of students")
...
Notice, that in this case we are grouping by three variables by simply speparating the variables that we want to group by with a comma in our `group_by()` function like this: `group_by(Group, year, n)`.
...
To evaluate this we will calculate the precentage of students using each flavor each year - this includes usage of any type of flavored tobacco product. Recall that NA values are not included in calculating the total count for our percentages. Therefore, these values reflect the percentage of each flavor for students reporting using favored products (I think... kinda confused). Also students were allowed to select more than one flavor.

```{r}
plot4 <- nyts_data %>%
  filter(year!=2015) %>%
  group_by(year) %>%
summarize(Menthol=(mean(menthol, na.rm = TRUE)*100),
         `Clove or Spice`=(mean(clove_spice, na.rm = TRUE)*100),
              Fruit=(mean(fruit, na.rm = TRUE)*100),
              Chocolate=(mean(chocolate, na.rm = TRUE)*100),
              `Alcoholic Drink`=(mean(alcoholic_drink, na.rm = TRUE)*100),
              `Candy/Desserts/Sweets`=(mean(candy_dessert_sweets, na.rm = TRUE)*100)) %>%
...
plot5 <- nyts_data %>%
  ...
  #          other == TRUE) %>%
# we want to remove students that used both e-cigarette and other tobacco products
  group_by(year, Group) %>%
summarize(Menthol=(mean(menthol, na.rm = TRUE)*100),
         `Clove or Spice`=(mean(clove_spice, na.rm = TRUE)*100),
          Fruit=(mean(fruit, na.rm = TRUE)*100),
