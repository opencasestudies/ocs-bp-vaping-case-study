...
4) Have vaping rates possibly influenced tobacco use?
...
```
Therefore, since we are using data from **2015-2019**, the data we are interested in are located in 5 different excel files, one for each year, each with its own [codebook](https://www.icpsr.umich.edu/icpsrweb/content/shared/ICPSR/faqs/what-is-a-codebook.html).
...
This was done all at once using the base `mapply()` function.
...
To add these grouping variables to our data we will do a bit more wrangling using the `mutate()` function again of the `dplyr` package. As discused above, our data set contains a set of questions that relate to whether the student has ever used the particular tobacco product (questions that start with the letter "E"), and questions that relate to whether the student currently uses the particular tobacco product (questions that start with the letter "C"). 
...
Finally, we are also interested in students that only use e-cigarettes and those that only use other forms of tobacco.
...
Now, let's look at identifying students who have only ever tried e-cigarettes, but are not current users, nor have they tried or do they currently use other tobacco products. We will again use the `case_when()` and the `mutate` function to accomplish creating new variables with specific values when certain conditions are met. In this case, we will specify that several statements of conditions  must be met by using the `&` symbol. Thus for a value of `TRUE` for the new `ecig_only_ever` variable, all of the conditions listed with `&` must be met.  If any of the last three conditions are `TRUE` than the `ecig_only_ever` value will be `FALSE` based on the last line `TRUE ~ FALSE`.
...
Now we will expand this to the other possible categories. In this case Note that since current users are a subset of ever users, it doesn't matter if a user reports yes to ever trying  e-cigarettes, they can still be a current user.
...
Ok, now we are going to make a `Group` variable based on the new variables we just made. In this case we will have a particular value based on one condition **or** another. This **or** conditionality is specified by the `|` symbol. Only one of the conditions needs to exist for that particular value, where as when we used the `&` symbol, all of the conditions had to be met. If a student has only ever tried or currently uses e-cigarettes, the value will be `Only e-cigarettes`. If a student has only ever tried or is a current user of other tobacco products the value will be `Only other products`. If the value of the no_use varaible is simply `TRUE`, then the `Group` variable value will be `Neither`. Finally, if a student has tried or currently uses both e-cigarettes or other tobacco products the `Group` variable value will be `Combination of products`. Therefore the values for the usage of the variables based on only using e-cigarettes or only other products will all be `FALSE`. Of course the student will also have had to answer yes to any of the use-based questions, thus the no_use variable would also need to be `FALSE`.
...
Lastly, it can be very helpful to have the total number of students surveyed each year. We can easily add a variable for this by using the `add_count()` function of the `dplyr` package. This will create a variable called `n` which will show the total counts based on the variable `year`.
...
To create a visualiztion about how tobacco usage has changed over time, we will first convert the usage data to a percent value for each year. To do this we will use the `group_by()` and `summarize` function of the `dplyr` package. This will create new variables which we will name `Ever` and `Current` based on the  percentages of `tobacco_ever` and `tobacco_current` for each year. In this case the `mean()` function is used to calculate the percentages based on using a value of `TRUE` as a count of of one and a value of `FALSE` as a count of zero. This function also automatically creates a total to divide by based on the total number of `TRUE` or `FALSE` values in the variable. NA values do not contibute to the total count. 
...
Nice! Now we can see how overall tobacco usage has changed since 2017. It appears that usage has increased a bit since 2017, and has surpassed the levels in 2015.
...
Thus, we will have four groups (e-cigarette ever users, e-cigarette current users,  ever tobbaco users, and current tobacco users) to plot, therefore, it would be useful to add color to our plot. Keep in mind that e-cigarette users are a subset of any tobbaco product users.

One important thing to keep in mind when creating plots, is that individuals with color blindness may have a difficult time distinguishing groups when certian color chocies are used. 

One great option is to use the `viridis` package, which offers color palattes with colors that are still distinguishable  by individuals with most forms of color blindness. 
...
We will calculate the mean ever and current usage percentages for students who used e-cigarette or any tobacco products (including e-cigarettes) for each year again using the `group_by()` function and the `summarize()` function. We will again use the `pivot_longer` function to convert our data to long format. We will also use the `separate()` function of the `tidyr` package to create two variables from one of variables. This is done by separating by, in this case, an underscore. 
...
We can see from this plot that the percentage of students who had only ever tried or currently used e-cigarettes was increasing since 2017, and at a faster rate than that of the usage of any tobacco products because the slope is more steeper.

Now let's plot students who only reported using either e-cigarettes or other tobacco products (besides e-cigarettes), but not both.  together on the same plot. In this case we will look at students who have only smoked either only e-cigarettes or only other tobacco products besides e-cigarettes.

Thus, we will have four groups (only e-cigarette ever users, only e-cigarette current users, only non-e-cigarette ever tobbaco users, and only non-e-cigarette current tobacco users) to plot.

We will calculate the mean ever and current usage percentages for students who had only used e-cigarette or other tobacco products (besides e-cigarettes) for each year again using the `group_by()` function and the `summarize()` function. We will again use the `pivot_longer` function to convert our data to long format. We will also again use the `separate()` function of the `tidyr` package to create two variables from one of variables. This is done by separating by, in this case, a space. 

```{r}

nyts_data %>%
    dplyr::group_by(year) %>%
    dplyr::summarize("Ever E-cigarette"=(mean(ecig_only_ever, na.rm = TRUE)*100),
                     "Current E-cigarette"=(mean(ecig_only_current, na.rm = TRUE)*100),
                     "Ever Non-e-cigarette" =(mean(non_ecig_only_ever, na.rm = TRUE)*100),
                     "Current Non-e-cigarette"=(mean(non_ecig_only_current, na.rm = TRUE)*100)) %>%
  pivot_longer(cols = -year, names_to = "User", values_to = "Percentage of students") %>%
  tidyr::separate(User, into = c("User", "Product"), sep = " ") %>%
  head()
...
plot1c <- nyts_data %>%
    ...
    dplyr::summarize("Ever E-cigarette"=(mean(ecig_only_ever, na.rm = TRUE)*100),
                     ...
                     "Current Non-e-cigarette"=(mean(non_ecig_only_current, na.rm = TRUE)*100))%>%
  pivot_longer(cols = -year, names_to = "User", values_to = "Percentage of students")%>%
  separate(User, into = c("User", "Product"), sep = " ") %>%
    ggplot(aes(x=year,y=`Percentage of students`, color = Product)) +
    geom_line(aes(linetype=User)) + 
  geom_point(show.legend = FALSE, size = 2) +
  # this allows us to choose what type of line we want for each line
...
Very intersting! We can see from this plot that the percentage of students who had only currently used (or ever tried) e-cigarettes  greatly increased since 2017, while in contrast the percentage of students who had only ever tried non-e-cigarette tobacco products actually diminished over time. In fact, we can see that in 2019 the percentage of students who were current e-cigarette users surpassed the percentage that had tried a non-e-cigarette product even just once. 
...
Notice, that in this case we are grouping by three variables by simply speparating the variables that we want to group by with a comma in our `group_by()` function like this: `group_by(Group, year, n)`. We will aslo rename the `year` variable to be upper case using the `rename()` function of the `tidyr` package.
...
We can see that luckily the majority of students did not report using any tobacco prodcuts. Of the students that did report using tobacco products, the majority of the students used both e-cigarettes and some other tobacco product. Again, a much larger percentage reported using only e-cigarettes rather than only other tobacco products in 2019.

We will explore the relaitonship between e-cigarette usage and other tobacco products a bit later in the case study.
...
We will calculate the mean e-cigarette ever and current percentages for each year and sex category again using the `group_by()` function and the `summarize()` function.  We will again use the `pivot_longer` function to convert our data to long format.
...
We are also interested in what vaping brands and flavors appear to be used the most frequently. Only the 2019 data has this information. Therefore, we will filter for just this year using the `filter()` function of  the `dplyr` package. We will use the `summarize()` function slightly differently this time, to calculate the total number of students using each brand `n` using the `n()` function and the `sum()` function to calculate the percent for each brand based on the counts. 

We will make a bar plot this time by using `geom_bar`. Importantly we assign the `stat` argument to `identity`, so that we are using the percentages that we calculated not the counts which is what is used by default. When color in specified outsied of the `aes()` argument, this determines the border color of the bars, which in this case will be black.
...
Juul appears to be the most widely used brand. This is in agreement with a recent [article](https://tobaccocontrol.bmj.com/content/tobaccocontrol/28/2/146.full.pdf):
...
We are also interested in how the usage of different flavors has changed over the years. 


To evaluate this we will calculate the precentage of students using each flavor each year - this includes usage of any type of flavored tobacco product. We will exclude 2015 data, as no specific flavor questions were yet asked at that time.

Recall that `NA` values are not included in calculating the total count for our percentages. However all of these flavor questions had complete reporting and did not have `NA` values. Therefore, these values reflect the percentage of students reporting using a particular favor out of all students surveyed (including those that did not use any tobacco products). Also students were allowed to select more than one flavor. You can look at if these variables had complete reporting by checking the `NA` values by using the base `summary` function. Alternatively you can create a visual representation using the `vis_miss()` function of the `naniar` package.
...
We can confirm with this plot that these variables have no `NA` values.
...
We will now look specificially at the usage of flavored e-cigarette products vs other flavored tobacco products. 

Recall that we made a variable called `Group` that identified students who used either just e-cigarette products, just other tobacco products (besides e-cigarettes), or students who used both e-cigarettes and some other type of tobacco product. We will compare the usage of these flavors for these different groups.
...
We can see from this plot that there has been an increase in the number of students reporting using flavored tabbaco products. Users who use both e-cigarettes and other tobacco products appear to report using flavored products the most, followed by users who only use e-cigarettes.

### Question 4

Have vaping rates possibly influenced tobacco use? Now we will investigate the usage of e-cigarrettes compared to other tobacco products in greater depth. 
...
Interesting! we can see that in 2019 the percentage of students that reported currently using e-cigarettes had surpased those that ever tried (even just once) a cigarette. Overall cigarrette usage appears to be declining over time. This is not the case for e-cigarettes.


Now we will look at students who reported that they had ever tried e-cigarettes or non-ecigarette products. In this case we will not separate out users who specifically only used one or the other. Therefore, the students included in this plot who reported as having ever tried e-cigarettes might also be  current users of non-e-cigarette products or may have at least tried non-e-cigaerette products.
...
Now we will use [survey weighting](http://www.applied-survey-methods.com/weight.html){target="_blank"} to see if this changes the general trends that we see in our data. This is a common technique used in survey data analysis because often the individuals that take a survey are not necessarily representative of the population that we are trying to gather information about.

For example, we may have more females that respond to the survey than males because perhaps female students were more willing to participate. In this case, the proportion of data values in our data will be smaller for the males than the proportion of actual male students and larger for the females than the true proportion of actual female students. To get a better estimate of overall e-cigarette smoking rates, the data from the males can be weighted based on the true proportion of male students to amplify the contribution of the responses from the males that did participate. Converesly, the female data can be weighted to diminish the contribution if their responses to the overall picture.

This involves making a weight based on the ratio of the proportion of survey responders of particular group and the actual proportion of of that group in the population.

For example, let's say that females account for 50% of the population and males account for 50 % of the population. Let's also say that 75% of the responders for the survey were female and only 25% were males. 

Then we could calculate survey weights using this formula:

$ \frac{\text{actual proportion of group in the population}}{\text{ proportion of group in the responders}}$  
...
Therefore each male response value would be multiplied by a factor of 2 and would have twice the contribution, while the female response values would have only about 70% of the contribution that they would have had otherwise.

Note that survey weights are in reality corrected for other aspects - for example the reponse rate to individual questions.

We do not need to caculate survey weights for our data as they were already supplied in the codebooks. 

We will now use the `srvyr` package to evaluate our data based on the survey weights that were provided in the respective codebooks for the data for each year.

Within the data you will see that we have variables called  `psu`, `finwgt`, and `stratum`.

Surveys like the one used to create the data we are using often sample people based on strata. This is done to ensure that the responses are representative of the population of interest. Thus, often people first think about ensuring that surveys are conducted in a variety of greographical areas. 
this is often called the **primary sampling unit** or **psu**. In [this survey](https://web.sph.harvard.edu/mch-data-connect/results/national-youth-tobacco-survey-nyts/){target="_blank"}, the county where the student's school was located was used as the psu. 

The `stratum` variable is a subset of the data that includes responders from different *PSUs*. Estimates for the values for repsonses are calculated using different strata allowing for improved percision of the response estimates.

The `finwgt` is the survey weight which was calulated based on a variety of factors.
...
For detailed information on such survey designs in genearl see [here](http://www.asasrms.org/Proceedings/y2008/Files/301835.pdf
http://ocw.jhsph.edu/courses/StatMethodsForSampleSurveys/PDFs/Lecture5.pdf){target="_blank"}.

We will use the `as_survey_desing()` function of the `srvyr`package to create a survey object with a specified survey design, in other words it includes information about how the survey was conducted.

The `strata` argument of the is used to specify the variables that defined strata in the data.

The `ids` argument is used to define cluster ids within the data - in this case we will use the `psu`.

The `weight` argument is the  used to define which variable(s) are the survey weights.

The `nest = TRUE` argument, forces cluster ids (in this case the psu) to be nested within the strata.

We can then use the `survey_mean()` function to calculate means of students who report using tobacco for each year while accounting for the survey design and weights. We will specify that we want [confidence interval](https://en.wikipedia.org/wiki/Confidence_interval) estimates by using the `vartype = "ci"` argument. The confidence intervals in our case give a range of possible values for the true population mean based on the data observed in the survey. We will multiply these values by 100 to get percentages.
...
We will now make a plot using this dat. The confidence intervals are included using the `geom_linerange()` function of the `ggplot2` package.
...
Now we will make a similar plot but this time we will look at students who reported ever trying or who currently use e-cigarettes as opposed to tobacco in general.
...
title_w <- ggdraw() + 
  draw_label(
    expression("Have e-cigarette rates possibly influenced tobacco use?"),
    fontface = 'bold',
    size=14,
    x = 0,
    hjust = 0
...
Now we are interested in looking at how tobacco usage changed for students of the same age. All of the data so far has included all 6th-12th graders every year. Now we will look at just the data for students expected to graduate in 2019. Thus these are the students who were in 8th grade in 2015. Most of these students were 9th graders in 2016 and so on. So first we will filter for the data for just the graduating class of 2019 students.
...
title_w_8 <- ggdraw() + 
  draw_label(
    expression("Among students who were in"~8^th~"grade in 2015, 
               have vaping rates possibly influenced tobacco use?"),
    fontface = 'bold',
    size=14,
    x = 0,
    hjust = 0
...
plotsC_title_final <- ggdraw() + 
  draw_label(
    expression("Prevalence of tobacco product use by product & user type"),
    size=14,
    x = 0.5) +
  theme(
    plot.margin = margin(0, 0, 0, 0)
...
My "centerpiece" plot is the middle plot in `final_plot`. The 8 plots around it help provide a very clear picture of what is going on in the US with regards to e-cigarette use and tobacco product use at large. On its own, it's difficult to understand the trends in the US and how important the weighting scheme is for inference. Once you add the left and right columns, it's clear what is going on. 
