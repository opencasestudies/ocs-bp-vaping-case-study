...
test %<>% mutate(ecig_only_ever = case_when(ecig_ever == TRUE &
...
What about e-cigarette use? How has e-cigarette usage changed over time?
...
We can see from this plot that the percentage of students who had only ever tried (or currently used) e-cigarettes was greatly increasing since 2017, while in contrast the percentage of students who had only ever tried non-e-cigarette tobacco products was  diminishing over time.
...
We will calculate the mean e-cigarette ever and current percentages for each year and sex category again using the `group_by()` function and the `summarize()` function. Notice, that in this case we are grouping by two variables. We will again use the `pivot_longer` function to convert our data to long format.
...
To evaluate this we will calculate the precentage of students using each flavor each year.
