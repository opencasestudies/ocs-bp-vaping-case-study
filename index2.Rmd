...
The first thing we do to create a plot is specify what data we are using for our x and y axes with the`aes()` argument of the `ggplot()` function. Then we add layers to our plot that specify what type of plot we would like to create. We can use the `geom_line()` function to create a lines for each type of user. We specify that we want to use different line types for each user using `aes()`. We will also add points to our lines using the `geom_point()` function.
...
We will calculate the mean e-cigarette ever and current percentages for each year and sex category again using the `group_by()` function and the `summarize()` function. We will use the `pivot_lo
...
plot2 <- nyts_data %>%
    ...
    filter(!is.na(Sex)) %>%
  #converting the columns names for Evern and Current into one column called Category and the values to a column called Percentage of students
  pivot_longer(cols = Ever:Current, names_to = "Category", values_to = "Percentage of students")
...
We are also interested in what vaping brands and flavors appear to be used the most frequently?
