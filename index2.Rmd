...
Finally, we are also interested in students that only use e-cigarettes and those that only other forms of tobacco.
We will again use the case_when function to accomplish creating new variables. 

Here is a toy example of what we will do with the larger dataset:
...
First we will make a small toy dataset:
```{r}
...
Now, let's look at identifying sutents who have only ever tried e-cigarettes, but are not current users, nor have they tried or currently use other tobacco products.
...
test <- test %>% mutate(ecig_only_ever = case_when(ecig_ever == TRUE & 
              ...
              #if any of the last three lines are TRUE than the ecig_only_ever value should be false
                                                    TRUE ~ FALSE))
test
```

We can see from the fourth row, that even though the student reported yes to ever trying e-cigarettes, they also reported yes to currently using e-cigarettes, thus the value for only ever trying e-cigarettes is `FALSE`. Additionally we can see from the seventh row that similarly even though the student reported yest to ever trying e0cigarettes, they also reported yes to ever trying other products, and the value for only ever trying e-cigarettes is `FALSE`. Importantly, we can see from the 6th row, that if all responses are negative than the value is `FALSE`.

Now we will expand this to the other possible categories. Notice since, current users are a subset of ever users, it doesn't matter if a user reports yes to ever trying  e-cigarettes, they can still be a current user.

```{r}
test <- test %>% mutate(ever_or_current_both = case_when(non_ecig_ever == TRUE &
                                                      non_ecig_current == TRUE |
                                                          ecig_current == TRUE |
                                                             ecig_ever == TRUE |
                                                      non_ecig_current == TRUE &
                                                          ecig_current == TRUE |
...
Ok, now we are going to make a `Group` variable based on the new variables we just made. If a student has only ever tried or currently uses e-cigarettes, the value will be `Only e-cigarettes`. The same will be true for other tobacco products. If a student has tried either they will be called `Both`.
...
Now let's plot e-cigarette usage and tobacco products usage (besides e-cigarettes) together on the same plot. In this case we will look at students who have only smoked either of e-cigarettes or only other tobacco products besides e-cigarettes.

In this case we will have four groups (only e-cigarette ever users, only e-cigarette current users, only non-e-cigarette ever tobbaco users, and only non-e-cigarette current tobacco users) to plot, therefore, it would be useful to add color to our plot.
...
We will calculate the mean ever and current usage percentages for students who had only used e-cigarette or other tobacco products (besides e-cigarettes) for each year again using the `group_by()` function and the `summarize()` function. We will again use the `pivot_longer` function to convert our data to long format. We will also use the `separate()` function of the `tidyr` package to create two variables from one of variables. This is done by separating by, in this case, a space. 
...
We can see from this plot that the percentage of students who had only ever tried (or currently used) e-cigarettes was greatly increasing since 2017, while in contrast the percentage of students who had only ever tried non-e-cigarette tobacco products was  diminishing over time. In fact, we can see that in 2019 the percentage of students who were current e-cigarette users surpassed the percentage that had tried a non-e-cigarette product even just once. 
...
We will calculate the mean e-cigarette ever and current percentages for each year and sex category again using the `group_by()` function and the `summarize()` function. Notice, that in this case we are grouping by two variables by simply using `group_by(year, Sex)`. We will again use the `pivot_longer` function to convert our data to long format.
...
To evaluate this we will calculate the precentage of students using each flavor each year - this includes usage of any type of flavored tobacco product.
...
From this plot, we can see that fruit flavors are the most widely used products, followed by menthol or mint flavored products. 
...
```{r}
count(nyts_data, Group)
```

The majority of the students that use tobacco use both e-cigarettes and other products.
