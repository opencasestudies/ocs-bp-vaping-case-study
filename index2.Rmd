...
OK, now we are going to make a `Group` variable based on the new variables we just made. In this case we will have a particular value based on one condition **or** another. This **or** conditional is specified by the `|` symbol. Only one of the conditions needs to exist for that particular value, where as when we used the `&` symbol, all of the conditions had to be met. If a student has only ever tried or currently uses e-cigarettes, the value will be `Only e-cigarettes`. If a student has only ever tried or is a current user of other tobacco products the value will be `Only other products`. If the value of the no_use variable is simply `TRUE`, then the `Group` variable value will be `Neither`. Finally, if a student has tried or currently uses both e-cigarettes or other tobacco products the `Group` variable value will be `Combination of products`. Therefore the values for the usage of the variables based on only using e-cigarettes or only other products will all be `FALSE`. Of course the student will also have had to answer yes to any of the use-based questions, thus the no_use variable would also need to be `FALSE`.
