...
4. Implementing Logistical regression with survey weighting avocado
...
avocado move this: To create these new variables, we will use the `mutate` function of the `dplyr` package. The `mutate` function can also be used to change the existing variables and create new variables from the old ones.
...
AVOCADO: Perhaps expand on function background, definition, arguments, return values, etc. here
...
avocado explanation of why .mssing is FALSE?
```{r}
...
title_uw <- ggdraw() + 
  draw_label(
    "Is there a relationship between vaping rates and tobacco use?",
    fontface = 'bold',
    size=14,
    x = 0,
    hjust = 0
...
AVOCADO: Replace all grepl with str_detect as seen above
...
In answer to our questions, we found that tobacco use has gone up slightly overall between 2015 and 2019, with little difference in rates of change comparing males to females. This slight increase is the result of a large increase in e-cigarette/vaping use, coupled with a decrease in use of other tobacco products. The most used brand of vaping/e-cigarette products is Juul, and fruit, menthol and candy/desserts/sweets are the most commonly used flavors.
