...
AVOCADO: Would it be simpler to do
tbl_names <- excel_files %>%
   str_extract("nyts201[5-9]")
below? I think all the square brackets could be confusing.
...
AVOCADO: What is the distinction between a tibble and a data.frame and a data_frame? I'm not sure that this distinction is useful to go into, but it is something that kind of bugs me, in case you do want to elaborate on it.
...
AVOCADO: For writing the csv files, shouldn't you just use map rather than introducing mapply? You also include an example using map, and it seems like it would be more in keeping with the tidyverse environment. I saw your response to this -- I am not that familiar with map, actually, so I didn't understand that the code didn't work. I'm fine with just using mapply, but you probably want to remove the map code in that case.
...
AVOCADO: Curious what the system time code is doing -- is it just left over from something before? It is not run or included in the markdown output.
...
AVOCADO: Same comment as above for:
...
AVOCADO: For the Sex variable, how did you determine "It's a bit difficult to tell if the encoding was the same across years" -- I don't see documentation of this (i.e., is this missing from the codebook somehow?), so it might be helpful to let the reader know how you figured this out.
...
AVOCADO: I think a little more illustration of the pre-data cleaning data would be helpful for understanding things like:
...
AVOCADO: I'm not sure why this reminder is where it is:
...
AVOCADO: I'm not sure where this should go, but I had the thought so I figured I'd include it. Maybe we can be more consistent about how we refer to students, subjects, respondents?
...
AVOCADO: This may be a tidyverse philosophical question, but case_when() is kind of overkill: you could just do tobacco_ever = tobacco_sum_ever > 0, since the latter is just going to return TRUE or FALSE, or NA as appropriate. But that requires understanding vectorization in a way that the case_when does not.
```{r}
nyts_data <-nyts_data %>%
...
AVOCADO: For the e-cigarette variables, can't you just use EELCIGT and CELCIGT? No need to do any summing or case_when here since it is just the one variable in any case. Also, see my comment above about not needing to use case_when to get the binary variable you want.
...
AVOCADO: In theory, non_ecig_ever == FALSE should imply non_ecig_current == FALSE, so is the last condition necessary? I'm not sure if the data are completely clean, i.e., are there some people who say they have never used, but who also say they are current users?
...
Thus, we will have four groups (e-cigarette ever users, e-cigarette current users,  ever tobacco users, and current tobacco users) to plot, therefore, it would be useful to add color to our plot. Keep in mind that e-cigarette users are a subset of any tobacco product users.
...
Thus, we will have four groups (only e-cigarette ever users, only e-cigarette current users, only non-e-cigarette ever tobacco users, and only non-e-cigarette current tobacco users) to plot.
...
Notice, that in this case we are grouping by three variables by simply separating the variables that we want to group by with a comma in our `group_by()` function like this: `group_by(Group, year, n)`. We will also rename the `year` variable to be upper case using the `rename()` function of the `tidyr` package.
...
We can see that luckily the majority of students did not report using any tobacco products. Of the students that did report using tobacco products, the majority of the students used both e-cigarettes and some other tobacco product. Again, a much larger percentage reported using only e-cigarettes rather than only other tobacco products in 2019.

We will explore the relationship between e-cigarette usage and other tobacco products a bit later in the case study.
...
To evaluate this we will calculate the percentage of students using each flavor each year - this includes usage of any type of flavored tobacco product. We will exclude 2015 data, as no specific flavor questions were yet asked at that time.
...
Have vaping rates possibly influenced tobacco use? Now we will investigate the usage of e-cigarettes compared to other tobacco products in greater depth. 
...
For example, we may have more females that respond to the survey than males because perhaps female students were more willing to participate. In this case, the proportion of data values in our data will be smaller for the males than the proportion of actual male students and larger for the females than the true proportion of actual female students. To get a better estimate of overall e-cigarette smoking rates, the data from the males can be weighted based on the true proportion of male students to amplify the contribution of the responses from the males that did participate. Conversely, the female data can be weighted to diminish the contribution if their responses to the overall picture.

This involves making a weight based on the ratio of the proportion of survey respondents of particular group and the actual proportion of of that group in the population.

For example, let's say that females account for 50% of the population and males account for 50 % of the population. Let's also say that 75% of the respondents for the survey were female and only 25% were males. 
...
We do not need to calculate survey weights for our data as they were already supplied in the codebooks. 

#### `srvyr` package and survey design 

***

We will now use the `srvyr` package to evaluate our data using survey weights that were provided in the respective codebooks for the data for each year.
...
2) The `stratum` variable is a subset of the data that includes respondents from different *PSUs*. Estimates for the values for responses are calculated using different strata allowing for improved precision of the response estimates.

3) The `finwgt` is the survey weight which was calculated based on a variety of factors.
...
For detailed information on such survey designs in general see [here](http://www.asasrms.org/Proceedings/y2008/Files/301835.pdf
...
1) The `strata` argument of the is used to specify the variables that defined strata in the data.
2) The `ids` argument is used to define cluster ids within the data - in this case we will use the `psu`.
...
We will now make similar plots for this subset of the data:
...
title_w_8 <- ggdraw() + 
  draw_label(
    expression("Among students expected to be in the graduating class of 2019, have vaping rates possibly influenced tobacco use?"),
    fontface = 'bold',
    size=14,
    x = 0,
    hjust = 0
...
Finally we will put our plots together to create a plot that describes the relationship between e-cigarette usage and overall tobacco use.
