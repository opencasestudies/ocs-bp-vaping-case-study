...
AVOCADO: The codebook link is not working for me. Also, I'm not sure why the same link is used three times here. I would think at least one of these links would be to an actual codebook from the survey being used. Or you can refer to the codebooks in the docs folder 
...
AVOCADO: This link does not work -- it looks like two different URLs? I have split it into two.
...
AVOCADO: Why are we calculating confidence intervals here, but not in the unweighted case? Is it just because the survey_mean function does it automatically? We could calculate it for the unweighted percentages as a statistical lesson (formula for confidence interval for a population proportion).
...
AVOCADO: I assume this plot title should change to something like: "What is the relationship between vaping rates and tobacco use?" 
