...
Recall that we are interested in investing how vaping product use has compared with other tobacco use over time. To answer this, we first want to get a sense of how tobacco use has changed in general since 2015. 
...
AVOCADO: Why leave in the commented lines below?
...
AVOCADO: That use of separate is very clever; I wonder if it might be getting lost in the plot command.
...
AVOCADO: Why the switch from _ to space for the separate command? Also, colors switch order below -- I think it would be better for e-cigarettes to be blue or purple consistently, right? Looks like it would be easier to switch the plots above to be consistent with the main figure.
...
Notice, that in this case we are grouping by three variables by simply separating the variables that we want to group by with a comma in our `group_by()` function like this: `group_by(Group, year, n)`. We will also rename the `year` variable to be upper case using the `rename()` function of the `tidyr` package. We do this just to make the plot label look nicer.

AVOCADO: The renaming is only for the sake of the plot label, right?
...
AVOCADO: There are NA values for Sex; should these be explicitly filtered out? Right now, they are not displayed on the plot I guess because they don't have a color assigned, but there is still an NA in the legend. I would probably just use the filter command to remove them from the data set passed to ggplot. Also, in this plot title e-cigarette is changed to vaping -- is that on purpose? Should we make that change earlier or throughout the case study? Or stick with e-cigarette?
...
AVOCADO: I think it might be worth pointing out the trick of reordering the factor by the values of another variable (which I actually did not know you could do). I added a sentence above.
...
The plot above confirms that these variables have no `NA` values.
