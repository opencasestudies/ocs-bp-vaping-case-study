...
You first need to specify that you are creating a functionby usingthe `function()` base function.
Yes, that's right it as a function for creating functions called function!

First we specify our input within the parantheses of `function()`. Thus if our function will apply something to an input called `x` then we would use `function(x)`. Really our input can be named whatever we want, but we we need to refer to it consistently within our function to indicate what we want done with the input data. We can actaully have more than one input as well, we would indicate two inputs like this: `function(x, b)`. Here we we would be using both `x` and `b` to do something in our function.
...
See Suggested Homework for some ideas of data analysis exercises to perform with this data.

AVOCADO: I know this section is a total mess; I am just outlining it before cleaning it up and making it better.

As an extension, we will include some material here on logistic regression and survey-weighted logistic regression that would be appropriate for answering Question 2 using statistical inference, rather than just data visualizations.

We can look at the final figure in the section on Question 2 and see that among both current and ever users of e-cigarettes, a higher percentage of males than females use or have used e-cigarettes. But what if we wanted to quantify this effect? This is where the tool of logistic regression can come in handy.
...
```{r}
# 2x2 table showing males and females by current vs not-current
```

One important ingredient in understanding the output of logistic regression is understanding the concept of an odds and an odds ratio. We can ask, among males who responded to the survey in 2015, what are the odds of being a current e-cigarette user? How about for females? How do these odds compare?

Why is it better to use odds rather than probability? This is a bit of a technical question, but it is an important ingredient of how logistic regression works. We will explain more down below.

OK, now we can introduce the idea of logistic regression.

We could answer this question using a 2x2 table:

|                                 | Male     | Female| Total|
|:--------------------------------|---------:|------:|-----:|
|Current e-cigarette user         |      8899|    187|  9086|
|Not current e-cigarette user     |       526|     71|   597|
|Total                            |      9425|    258|  9683|
...
Logistic regression is widely used to when the response variable is binary; in our case, the binary response is whether an individual is a current e-cigarette user.  The standard logistic regression equation can be written as:

$logit(p)= log(\frac{p}{1-p})=X^{T}\beta$, where $p=P(Y=1)=E(Y)$ 

where we define p as the probability that a student respondent is a current e-cigarette user, and Y is the binary variable that is 1 when a student respondent is a current e-cigarette user. Here, we are trying to measure the relationship between some input variables $X$ and the outcome $Y$ using a linear 
...
We used this data to answer the questions:
...
If you are intersted in an infographic summarization of the 2019 findings, and links to many more resources about this topic and data set, see the FDA's website [here](https://www.fda.gov/tobacco-products/youth-and-tobacco/youth-tobacco-use-results-national-youth-tobacco-survey){target="_blank"}.
