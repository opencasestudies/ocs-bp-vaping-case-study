...
Here we also used the `%>%` pipe which can be used to pass the input from one function to another for later sequential steps. You can use pipes if you load the `dplyr` package package.
...
```{r}
nyts_data <- nyts_data %>% dplyr::add_count(year)
```
...
test <- test %>% mutate(var1 = as.logical(var1))
test
