...
knitr::include_graphics(here::here("www", "img", "final_plot.png"))
```
...
knitr::include_graphics(here::here("www", "img", "context.png"))
```
...
```{r, echo = FALSE, fig.align ="center"}
include_graphics(here::here("www", "img", "e-cigarette-aerosol-can-contain-harmful-ingredients.jpg"))
```
...
knitr::include_graphics(here::here("www", "img", "lung.png"))
```
...
knitr::include_graphics(here::here("www", "img", "data.png"))
```
...
knitr::include_graphics(here::here("www", "img", "excel.png"))
```

The codebook explains what the variables (the columns) are:
```{r, echo = FALSE, fig.align ="center", out.width = "600 px"}

knitr::include_graphics(here::here("www", "img", "variables.png"))
```
...
knitr::include_graphics(here::here("www", "img", "qn1.png"))
```
...
load_reduced("ocs-bp-vaping-case-study", outpath = getwd())
```
...
```{r, echo=FALSE}
save(nyts_data, file = here::here("www", "data", "imported", "imported_data.rda"))
```
...
If you have been following along but stopped, we could load our imported data using the `OCSdata` package like so:

```{r, eval=FALSE}
# install.packages("devtools")
# devtools::install_github("opencasestudies/OCSdata")

load_imported_data("ocs-bp-vaping-case-study", outpath = getwd())
```

If you prefer loading the imported data from the "data" directory, you may do it like so:
...
Then, you may load the imported data using the following function:
...
knitr::include_graphics(here::here("www", "img", "grade.png"))
```
...
```{r, eval=FALSE}
save(nyts_data, file = here::here("www", "data", "wrangled", "wrangled_data.rda"))

write_csv(nyts_data, file = here::here("www", "data", "wrangled", "nyts_data.csv"))
```
...
quiz(caption = "",
  question("Suppose that we want to change value \"a\" in a column(variable) of a dataset to value \"b\" , which one of the following functions of the `dplyr` package should be used?",
    answer("`rename()`", message = "`rename()` renames a column, not a value in that column."),
    answer("`recode()`", correct = TRUE),
    allow_retry = TRUE,
    random_answer_order = TRUE
  ),
  question("Which of the following can the `mutate()` function in the `dplyr` package do? (check all that apply)",
...
Suppose there is a list of datasets called `norm_data`. The first dataset in `norm_data` contains observations from a normal distribution with mean 0. The second contains observations from a normal distribution with mean 5. For both of them, the observations in the first column `var1` are from a distribution with variance 1. The observations in the second column `var2` are from a distribution with variance 4.

Write a function called `rename_sd` that renames the first column as "SD = 1", and the second column as "SD = 2". Then, apply this function to the list `norm_data`.
...
```{r dew_exercise-hint-1}
# What input argument should the function take?
# What should the function do with the input argument?
```

```{r dew_exercise-hint-2}
# What input argument should the function take?
# What should the function do with the input argument?

# What function can be used to apply a function to each element in our list?
```

```{r dew_exercise-hint-3}
# write the function
...
If you have been following along but stopped, we could load the wrangled data using the `OCSdata` package like so:

```{r, eval=FALSE}
# install.packages("devtools")
# devtools::install_github("opencasestudies/OCSdata")

load_wrangled_data("ocs-bp-vaping-case-study", outpath = getwd())
```

If you prefer loading the wrangled data from the "data" directory, you may do it like so:
...
***
<details> <summary> If you skipped the data exploration and wrangling section click here. </summary>
...
Then, you may load the wrangled data using the following function:

```{r, eval=FALSE}
load_wrangled_data("ocs-bp-vaping-case-study", outpath = getwd())
```

If the package does not work for you, alternatively, an RDA file (stands for R data) of the data can be found in our [GitHub repository](https://github.com//opencasestudies/ocs-bp-vaping-case-study/tree/master/data/wrangled) or slightly more directly [here](https://raw.githubusercontent.com/opencasestudies/ocs-bp-vaping-case-study/master/data/wrangled/wrangled_data.rda). Download this file and then place it in your current working directory within a subdirectory called "wrangled" within a subdirectory called "data" to copy and paste our code. We used an RStudio project and the [`here` package](https://github.com/jennybc/here_here) to navigate to the file more easily. 
...
We can also see from the fourth row, that even though the student reported yes to ever trying e-cigarettes, because they also reported yes to currently using e-cigarettes the value for only ever trying e-cigarettes is `FALSE`. Additionally we can see from the seventh row that similarly even though the student reported yes to ever trying e-cigarettes, they also reported yes to ever trying other products, and the value for only ever trying e-cigarettes is `FALSE`. Importantly, we can see from the 6th row, that if all responses are negative than the value is `FALSE`.
...
```{r, eval=FALSE}
save(nyts_data, file = here::here("www", "data", "wrangled", "wrangled_data_with_var_for_plots.rda"))

readr::write_csv(nyts_data, 
                 file = here::here("www", "data", "wrangled", "nyts_data_for_plots.csv"))
```
...
***
<details> <summary> Click here if you skipped the previous sections and want to start here. </summary>
...
Then, you may load the wrangled data for plots using the following function:

```{r, eval=FALSE}
load_wrangled_data_with_var_for_plots("ocs-bp-vaping-case-study", outpath = getwd())
```

If the package does not work for you, alternatively, an RDA file (stands for R data) of the data can be found in our [GitHub repository](https://github.com//opencasestudies/ocs-bp-vaping-case-study/tree/master/data/wrangled) or slightly more directly [here](https://raw.githubusercontent.com/opencasestudies/ocs-bp-vaping-case-study/master/data/wrangled/wrangled_data_with_var_for_plots.rda). Download this file and then place it in your current working directory within a subdirectory called "wrangled" within a subdirectory called "data" to copy and paste our code. We used an RStudio project and the [`here` package](https://github.com/jennybc/here_here) to navigate to the file more easily. 
...
```{r, echo=FALSE}
load(here::here("www", "data", "wrangled", "wrangled_data_with_var_for_plots.rda"))
```
...
ggsave(here::here("www", "img", "plot2.png"))
```
...
knitr::include_graphics(here::here("www", "img", "HuangJ_DuanZ_KwokJ_et_al_TobaccoControl_Figure1.png"))
```
...
Now we will put these plots together using the `plot_grid()` function of the `cowplot` package.  We will also modify the labels using the `ggdraw()` function, which is also part of the `cowplot` package. To learn more about the `cowplot` package, refer to this [case study](https://www.opencasestudies.org/ocs-bp-RTC-analysis/#Data_Visualization){target="_blank"}.
...
```{r dv_quiz1, echo=FALSE}
question("Which of the following is the long format of the data?",
    answer("A", message = "This is the wide format."),
...
```{r, echo=FALSE}
exercise_dv1 <- data.frame(ID = c(1, 2), Year = c(2015, 2018), Length = c(20, 21.5))
exercise_dv1
```
...
```{r dv_quiz3, echo=FALSE}
question("Suppose a survey was conducted in Maryland where patients with type 2 diabetes were sampled. The researchers first sampled counties in the state, and then hospitals in each county. In addition, individuals were grouped based on gender during sampling. Which of the following is the PSU (primary sampling unit) in this survey design?",
    answer("Hospitals in each county", message = "This is not the **primary** sampling unit."),
    answer("Gender", message = "This is the stratum."),
    answer("Counties in the state", correct = TRUE),
    allow_retry = TRUE,
    random_answer_order = TRUE
  )
```

Use the R built-in dataset `mtcars` to complete the following exercise.
...
1. Only keep the `cyl` and `vs` variables of the data.
2. Create a new variable called `vshaped`, whose value is `TRUE` when `vs` = 0 and `FALSE` otherwise.
3. Break the cars (observations) into groups based on their `cyl` number and find out the proportion of cars that have v-shaped engine in each group.
...
```{r dv_exercise1, exercise=TRUE, eval=FALSE}
# Note there might be other ways to do this that give us the same results 
# For practice purposes try to fill in the blanks

mtcars %>%
  # step 1
  dplyr::______(cyl, vs) %>%
  # step 2
  dplyr::mutate(vshaped = _________(vs == 0 ~ TRUE, ____ ~ FALSE)) %>%
  # step 3
  dplyr::________(cyl) %>%
  _________(Proportion = (mean(_______) * 100))
```
...
mtcars %>%
  dplyr::select(cyl, vs) %>%
  dplyr::mutate(vshaped = case_when(vs == 0 ~ TRUE, TRUE ~ FALSE)) %>%
  dplyr::group_by(cyl) %>%
  summarize(Proportion = (mean(vshaped) * 100))

# you need to write your own code for submission 
# If you copy and paste the solution here and submit it, there may be an error where R does not recognize the pipe operator
```

```{r dv_exercise1-check, eval=FALSE}
grade_code()
```

Suppose that we have a dataset called `exercise.weight`. In this dataset, the `group` variable indicates treatment and control groups; the `time` variable indicates measurement time (in days); and the `avg.weight` variable indicates the average weight in the group. Write some code to plot weight change by time with different line-types representing different groups.\
We want the plot to:

* have title as "Group Average Weight Change"
* have $x$ axis name as "Time (day)" and $y$ axis name be "Group Average Weight (lb)"
* have both points and lines
* use solid line for the control group and dashed line for treatment group
* have legend at the bottom of the plot explaining line types

```{r, dv_exercise2-setup}
library(tidyverse)

exercise.weight <- data.frame("group" = c("control", "control", "control", "treatment", "treatment", "treatment"),
                     "time" = c(0, 15, 30, 0, 15, 30),
                     "avg.weight" = c(170, 167, 165, 172, 165, 155))
```

```{r dv_exercise2, exercise=TRUE, exercise.eval=TRUE, exercise.lines = 5}
exercise.weight
```

```{r dv_exercise2-solution}
exercise.weight %>%
  ggplot(aes(x = time, y = avg.weight)) +
  geom_point() +
  geom_line(aes(linetype = group)) +
  labs(title = "Group Average Weight Change",
       x = "Time (day)",
       y = "Group Average Weight (lb)") +
  # the scale_linetype_manual function is not necessary in this case, but may be needed in other cases
  scale_linetype_manual(values = c(1, 2), 
                        breaks = c("control", "treatment")) +
  theme(legend.position = "bottom")
```

```{r dv_exercise2-check}
grade_this_code()
```
...
If you have been following along but stopped, we could load the wrangled data for plots using the `OCSdata` package like so:
...
```{r, echo = FALSE, out.width = "800 px"}
knitr::include_graphics(here::here("www", "img", "plot2.png"))
```
...
We can also look at the other columns of the model output to assess whether our data indicate that $\beta_1$ is statistically significantly different from 0. The p-value for the `Sex` variable in our model is `r format(currEcigSexTidy %>% filter(term == "Sexfemale") %>% pull(p.value), digits = 3)`. Since this value is < 0.05, we would reject the null hypothesis that $\beta_1 = 0$ based on our model output. See this [case study](https://www.opencasestudies.org/ocs-bp-rural-and-urban-obesity/#Alpha){target="_blank"} for more information about alpha and p-values.
...
Suppose we fitted a logistic regression model:

$$logit(\text{probability of current e-cigarette use}) = \beta_0 + \beta_1 \cdot Female + \beta_2 \cdot Age$$
where $Female = 1$ indicates female, $Female = 0$ indicates male, and $Age$ is a continuous variable.

```{r da_quiz, echo=FALSE}
question("From the fitted model, which of the following options estimates the odds ratio of current e-cigarette use, comparing a 15-year-old female to a 15-year-old male?",
    answer("$\\hat \\beta_1$", message = "This is log odds ratio, not odds ratio."),
    answer("$exp(\\hat \\beta_1)$", correct = TRUE),
    answer("$exp(\\hat \\beta_1 + 15 \\hat \\beta_2)$", message = "Coefficient of age got canceled out during calculation."),
    allow_retry = TRUE,
    random_answer_order = TRUE
  )
```
...
```{r, echo=FALSE, include=FALSE}
ggsave(here::here("www", "img", "final_plot.png"))
```
...
If you are interested in an info-graphic summary of the 2019 findings, and links to many more resources about this topic and dataset, see the FDA's website [here](https://www.fda.gov/tobacco-products/youth-and-tobacco/youth-tobacco-use-results-national-youth-tobacco-survey){target="_blank"}.
