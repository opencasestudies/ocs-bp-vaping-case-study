...
***
Since these excel files are so large (each has roughly 20,000), it takes a bit of time for the data to load. To make the process faster, we previously imported these files, selected only our questions of interest, and saved this data as csv files. 
...
Recallfrom the codebook for this year, that age isn't listed perhaps in the way one might expect:
...
We also need to add new variables about usage of brands and flavors of e-cigrattes, because although later years have questions about this, this year unfortunately does not. Eventually we want to put the data from each year together, so we need the same variables for each year. Thus we need to add variabels that just say NA for the brand and flavor data for this year.
